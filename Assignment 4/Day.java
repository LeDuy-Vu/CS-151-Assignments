...
public class Day
{
	 ...
	 */
	public Day(int year, int month, int date) throws IllegalArgumentException
	{
		checkYear(year) ;
		checkMonth(month) ;
	...
	/**
	 * Checks if the input year is invalid.
	 * @param year the input year
	 * @throws IllegalArgumentException if year == 0
	 */
	private void checkYear(int year) throws IllegalArgumentException
	{
		if (year == 0)
			throw new IllegalArgumentException("Year should not be 0!") ;
	}
	
	/**
	 * Checks if the input month is invalid.
	 * @param month the input month
	 * @throws IllegalArgumentException if month is not between 1 and 12
	 */
	private void checkMonth(int month) throws IllegalArgumentException
	{
		if (month < 1 || month > 12)
	...
	/**
	 * Checks if the input date is invalid.
	 * @param year the input year
	 * @param month the input month
	 * @param date the input date
	 * @throws IllegalArgumentException if date is invalid depending on the year and month
	 */
	private void checkDate(int year, int month, int date) throws IllegalArgumentException
	{
		if (date < 1 || date > 31)
			throw new IllegalArgumentException("Date should be between 1 and 31!") ;
		
		if (year == 1582 && month == 10 && date > 4 & date < 15)
			throw new IllegalArgumentException("In October 1582, the 4th is followed immediately by the 15th!") ;
		...
		if (month == 2)
		{
			...
			{
				if (date > 28)
					throw new IllegalArgumentException("February of a non-leap year should not have more than 28 days!") ;
			}
			else
			{
				if (date > 29)
					throw new IllegalArgumentException("February of a leap year should not have more than 29 days!") ;
			}
		}
		else if (month == 4 || month == 6 || month == 9 || month == 11)
		{
			if (date == 31)
				throw new IllegalArgumentException("April, June, September, and November should not have more than 30 days!") ;
		}
	}
}
